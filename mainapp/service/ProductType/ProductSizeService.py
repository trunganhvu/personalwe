...
def get_all_product_size_in_product_type(product_type_id):
    ...
    """
    key_cache = KEY_CACHE_GET_ALL_PRODUCT_SIZE_IN_TYPE + str(product_type_id)
    ...
    if not cached_data:
        ...
        if product_size_list.count() > 0:
            # Set into cache
            cache.set(key_cache, product_size_list, settings.CACHE_TIME)
...
def insert_product_size(product_size):
    ...
    # Clean cache
    CacheUtil.clean_cache_by_key(KEY_CACHE_GET_ALL_PRODUCT_SIZE_IN_TYPE + str(p_size.product_type_id_id))
...
def delete_product_size_by_id(product_type_id):
    ...
    if p_size is not None:
        ...
        # Clean cache
        CacheUtil.clean_cache_by_key(KEY_CACHE_GET_ALL_PRODUCT_SIZE_IN_TYPE + str(p_size.product_type_id)) 
